...
public class PlaylistResultScrollListener extends RecyclerView.OnScrollListener {

    private static final String TAG = PlaylistResultScrollListener.class.getSimpleName();
    private static final int SCROLL_BUFFER = 3;

    private final LinearLayoutManager mLayoutManager;
    private int mCurrentItemCount = 0;
    private boolean mAwaitingItems = true;
    private UserPlaylistPager mPlaylistPager;
    private UserPlaylistPager.CompleteListener mPlaylistListener;

    public PlaylistResultScrollListener(LinearLayoutManager layoutManager, UserPlaylistPager playlist, UserPlaylistPager.CompleteListener listener) {
        mLayoutManager = layoutManager;
        mPlaylistPager = playlist;
    ...
    public void onScrolled(RecyclerView recyclerView, int dx, int dy) {
        ...
        if (!mAwaitingItems && itemPosition + 1 >= itemCount - SCROLL_BUFFER) {
            mAwaitingItems = true;
            mPlaylistPager.getNextPageSearch(mPlaylistListener);
        }
    }
}
